...
import {
  ArrowLeft, Wallet, Presentation,
  LayoutGrid, History, ShieldCheck
} from "lucide-react";
import Link from "next/link";
import { notFound } from "next/navigation";

interface ClassFinancePageProps {
  params: Promise<{
    classId: string;
  }>;
}

export default async function ClassFinancePage({ params }: ClassFinancePageProps) {
  const { classId } = await params;

  // 1. Fetch Class and its members with their payment history
    ...
    include: {
      members: {
        include: {
          ...
          paymentReceipts: {
            where: {
              status: "CONFIRMED", // Only confirmed payments reduce debt
            },
            select: {
              amount: true,
            },
  ...
  // 2. Map data for the form with calculated debt
